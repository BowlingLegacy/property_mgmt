...
class LiveFlowTests(TestCase):
    ...
    @patch("main.views.timezone.localdate", return_value=date(2026, 7, 17))
    def test_aaron_collection_watch_after_400_payment(self, _mocked_localdate):
        landlord = User.objects.create_user(
            username="aaron-payment-landlord",
            email="aaron-payment-landlord@example.com",
            password="StrongPass123!",
            role="landlord",
            is_staff=True,
        )
        property_obj = Property.objects.create(name="Aaron Payment Property", landlord_email=landlord.email)
        resident_user = User.objects.create_user(username="aaron-payment-user", password="StrongPass123!", role="tenant")
        resident = HousingApplication.objects.create(
            property=property_obj,
            user=resident_user,
            full_name="Aaron Brown",
            email="aaron-payment@example.com",
            age=40,
            space_label="A",
            monthly_rent=Decimal("650.00"),
            balance=Decimal("0.00"),
        ...
        self.client.login(username="aaron-payment-landlord", password="StrongPass123!")
        response = self.client.get(reverse("landlord_dashboard"))
        row = next(row for row in response.context["collection_watch_rows"] if row["application"].id == resident.id)

        self.assertEqual(row["prior_rent_balance"], Decimal("0.00"))
        self.assertEqual(row["rent_paid"], Decimal("50.00"))
        self.assertEqual(row["rent_due"], Decimal("600.00"))
        self.assertEqual(row["utility_due"], Decimal("55.00"))
        self.assertEqual(row["rent_due"] + row["utility_due"], Decimal("655.00"))
    ...
    def test_commercial_custom_reports_use_scoped_property_data(self):
        ...
        self.assertContains(delinquency, "Rent Expected")
        self.assertContains(delinquency, "Utilities Due")
    ...
    def test_staff_can_delete_duplicate_setup_attempt_connected_to_completed_login(self):
        landlord = User.objects.create_user(
            username="delete-duplicate-completed-intake",
            email="delete-duplicate-completed-intake@example.com",
            password="StrongPass123!",
            role="landlord",
            is_staff=True,
        )
        resident_user = User.objects.create_user(
            username="completed-duplicate-resident",
            email="completed-duplicate@example.com",
            password="StrongPass123!",
            role="tenant",
        )
        property_obj = Property.objects.create(name="Completed Duplicate Property", landlord_email=landlord.email)
        application = HousingApplication.objects.create(
            property=property_obj,
            user=resident_user,
            full_name="Completed Resident",
            phone="555-0407",
            email="completed-duplicate@example.com",
            age=50,
            income_source="Existing resident intake",
        ...
        bad_intake = ExistingResidentIntake.objects.create(
            property=property_obj,
            application=application,
            first_name="Bad",
            last_name="Attempt",
            email="wrong-completed@example.com",
            phone="555-0407",
            room_unit_label="C",
        )
        good_intake = ExistingResidentIntake.objects.create(
            property=property_obj,
            application=application,
            first_name="Completed",
            last_name="Resident",
        ...
        self.client.login(username="delete-duplicate-completed-intake", password="StrongPass123!")
        response = self.client.post(reverse("delete_existing_resident_intake", args=[bad_intake.id]))

        self.assertRedirects(response, reverse("landlord_attention"))
        self.assertFalse(ExistingResidentIntake.objects.filter(id=bad_intake.id).exists())
        self.assertTrue(ExistingResidentIntake.objects.filter(id=good_intake.id).exists())
        self.assertTrue(HousingApplication.objects.filter(id=application.id, user=resident_user).exists())

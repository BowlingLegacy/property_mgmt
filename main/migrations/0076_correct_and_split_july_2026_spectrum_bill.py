...
def correct_and_split_july_spectrum(apps, schema_editor):
    ...
    receipt = AccountingReceipt.objects.filter(
        ...
        receipt_file__icontains="CHARTER_7-2-26",
    ).first()
    ...
    receipt.amount = Decimal("291.38")
    receipt.description = "July 2026 Spectrum services"
    receipt.save(update_fields=["financial_entry", "category", "amount", "description"])
...
class Migration(migrations.Migration):
    dependencies = [("main", "0075_retry_split_september_charter_receipt")]

    operations = [migrations.RunPython(correct_and_split_july_spectrum, noop_reverse)]
